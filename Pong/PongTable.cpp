...
PongTable::PongTable() {
	...
	computerPaddle = PongObject(PADDLE_HEIGHT, PADDLE_WIDTH, aiPaddlePosition, { 0,0 }, {0,0}, false);
...
bool PongTable::collosions() {
	...
	if (ball.intersects(&computerPaddle)) {
		ballCurrent.xValue = computerPaddle.getCurrent().xValue - BALL_WIDTH - 1;
		ballVelocity.xValue *= -1;
		computerPaddle.setIsDirty(true);
	}
	else if (ball.intersects(&playersPaddle)) {
		ballCurrent.xValue = playersPaddle.getCurrent().xValue + PADDLE_WIDTH + 1;
		ballVelocity.xValue *= -1;
	...
	computerPaddle.setVelocity(aiPadVel);
...
void PongTable::moveComputerPaddle() {
	...
	Position ballCurrent = ball.getCurrent();
	Position aiPadPos = computerPaddle.getCurrent();
	Position aiPadVel = computerPaddle.getVelocity();
